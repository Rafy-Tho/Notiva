...
- **Editor toolbar:** `sticky top-0 z-10 flex items-center gap-0.5 overflow-x-auto overflow-y-hidden border-b border-border bg-background/80 backdrop-blur px-3 py-1.5` (single scrollable row on narrow screens; buttons `shrink-0`); group separators `mx-1 h-4 w-px bg-border` (separators also `shrink-0`); active tool `bg-muted text-foreground`; container has `role="toolbar"` + `aria-label="Formatting options"`. Tools (in order): Undo/Redo, **block-type `DropdownMenu`** (Paragraph / H1 / H2 / H3 / Blockquote / Code block — replaces the old heading button trio), Bold/Italic/Underline/Strikethrough/Inline code, **link** (Radix `Popover` with URL input; auto-prefixes `https://`; Apply/Remove), Bullet/Ordered/Task lists, Blockquote/Code block/Horizontal rule, **text-alignment `DropdownMenu`** (left/center/right/justify + unset), **image** (Radix `Popover` with URL input; paste-to-insert hint), and `TableIcon` → insert-table, or "Table options" `DropdownMenu` when the cursor is inside a table (insert row above/below, insert column left/right, merge cells, split cell, toggle header row/column, delete row/column/table). Tooltips include shortcut hints (e.g. `⌘B`).
- **Note status bar (NoteDetailPage):** sticky bottom hairline (`shrink-0 border-t border-border bg-background/80 backdrop-blur`) aligned to the editor content column via the shared `px-4 sm:px-6 md:px-10 lg:px-12` padding; left cluster `h-8` shows live stats — word count, character count, reading time — in `text-[11px] tabular-nums muted`; right cluster shows a compact save state (spinner "Saving…/Updating…", `text-destructive` "Save failed"/"Conflict", `text-warning` "Unsaved", `text-success` "Saved", muted "Draft") and a **Focus-mode toggle** (Maximize2/Minimize2 icon, `hidden sm:inline` label) with keyboard hint `⌘/`.
- **Editor metadata line:** under the note title, `text-[11px] text-muted-foreground` with `·` separators — "Edited Xm ago · Created MMM d, yyyy · X min read".
- **Focus mode:** session-only (`useUIStore.focusMode`, **not** persisted) — hides the `AppHeader`, note list column, list toggle strips, note meta/actions row, and the editor toolbar (`NoteEditor showToolbar={false}`); title gets extra top padding (`pt-10`) and remains visible; exit via the status-bar toggle, `⌘/`, or `Esc`.
